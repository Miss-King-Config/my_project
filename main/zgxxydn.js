...
//获取host
async function Host() {
    let host = random(0, 255) + "." + random(0, 255) + "." + random(0, 255) + "." + random(0, 255);
    while (true) {
                    ...
                    if (err) {
                        ...
                        if (data2.code == 0) {
                            ...
                            if (country == "中国") {
                                ...
                                host = random(0, 255) + "." + random(0, 255) + "." + random(0, 255) + "." + random(0, 255);
                            }
                        } else {
                            host = random(0, 255) + "." + random(0, 255) + "." + random(0, 255) + "." + random(0, 255);
                        }
...
async function receiveRed(id) {
    return new Promise((resolve) => {
        let url = {
            url: `https://xmr.shqnon.com/hd/?ct=TuiRedInteract&ac=receiveRed&game_id=331&game_pkg=yjsydlw_pxzgxxydn_CK&role_id=${role_id}&uid=${user_id}&partner_id=1&config_id=${config_id}&sign=${sign}&sdk_token=${sdk_token}&red_id=${id}&role_name=${encodeURIComponent(role_name)}&word_desc=${word_desc}&ad_code=pxzgxxydn_11194_63733_2477_222_1436954_ad`,
            headers: {
                'x-forwarded-for': `${host}`,
                'cookie': ''
            }
