...
[task_local]
#中信优享+
0 0 0,6,12,18 * * *, tag=中信优享+, img-url=, enabled=true

================Loon==============
[Script]
cron "0 0 0,6,12,18 * * *", script-path=, tag=中信优享+

===============Surge=================
中信优享+ = type=cron, cronexp="0 0 0,6,12,18 * * *", wake-system=1, timeout=33600, script-path=

============小火箭=========
中信优享+ = type=cron, script-path=, cronexpr="0 0 0,6,12,18 * * *", timeout=33600, enable=true
*/
            ...
            try {
                if (err) {
                    ...
                    if (data2.statusCode == 200) {
                        ...
                        await $.wait(1000); //等待1秒
                        await user_userInfo(access_token);
                        await $.wait(1000); //等待1秒
...
//获取uid
async function user_userInfo(access_token) {
