...
[task_local]
#央视频签到
00 00 8,11 * * * , tag=央视频签到, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 8,11 * * *" script-path=,tag=央视频签到

===============Surge=================
央视频签到 = type=cron,cronexp="00 00 8,11 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
央视频签到 = type=cron,script-path=, cronexpr="00 00 8,11 * * *", timeout=33600, enable=true
*/

const $ = new Env('央视频签到');
$.log(`需要新建环境变量: Yspqd_Cookie\n填写抓包Cookie\n多用户可以用"#" "@" "\\n" 隔开`);
...
async function checkEnv() {
    const Yspqd_Cookie = ($.isNode() ? (process.env.Yspqd_Cookie) : ($.getval('Yspqd_Cookie'))) || "";
    if (!Yspqd_Cookie) {
        let str = Yspqd_Cookie ? "" : "Yspqd_Cookie";
        $.log(`未找到环境变量: ${str}\n`);
        return false;
    }
    if (Yspqd_Cookie.indexOf('#') != -1) {
        appUrlArrs = Yspqd_Cookie.split('#');
        $.log(`您选择的是用"#"隔开 Yspqd_Cookie\n`);
    } else if (Yspqd_Cookie.indexOf('\n') != -1) {
        appUrlArrs = Yspqd_Cookie.split('\n');
        $.log(`您选择的是用"\\n"隔开 Yspqd_Cookie\n`);
    } else if (Yspqd_Cookie.indexOf('@') != -1) {
        appUrlArrs = Yspqd_Cookie.split('@');
        $.log(`您选择的是用"@"隔开 Yspqd_Cookie\n`);
    } else {
        appUrlArrs = [Yspqd_Cookie];
    }
...
async function initAccountInfo() {
    for (numUser = 0; numUser < totalUser; numUser++) {
        ...
        await sign_component_page("149734527595216");
...
async function sign_component_page(signOperatingId) {
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
            headers: {
                "Referer": `https://m.yangshipin.cn/static/duiba.html?redirect=https%3A%2F%2Factivity.m.duiba.com.cn%2Fsign%2Fcomponent%2Fpage%3FsignOperatingId%3D${signOperatingId}`,
                "Cookie": `${Cookie}`
            }
        };
...
async function getToken(key, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/chw/ctoken/getToken`,
            body: `timestamp=${timestamp}`,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
                "Cookie": `${Cookie}`
            }
        };
...
async function sign_component_doSign(token, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/sign/component/doSign?_=${timestamp}`,
...
async function sign_component_signResult(orderNum, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/sign/component/signResult?orderNum=${orderNum}&_=${timestamp}`,
            headers: {
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
                "Cookie": `${Cookie}`
            }
...
async function sign_component_index(key, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/sign/component/index?signOperatingId=${signOperatingId}&preview=false&_=${timestamp}`,
            headers: {
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
                "Cookie": `${Cookie}`
            }
...
async function sign_component_doJoin(token, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/sign/component/doJoin?_=${timestamp}`,
            body: `signOperatingId=${signOperatingId}&token=${token}`,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
...
async function getOrderStatus(orderNum, signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/plugin/getOrderStatus?orderId=${orderNum}&_=${timestamp}`,
            headers: {
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
                "Cookie": `${Cookie}`
            }
...
async function ctool_getCredits(signOperatingId) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://78211.activity-17.m.duiba.com.cn/ctool/getCredits?_=${timestamp}`,
            body: ``,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                "Referer": `https://78211.activity-17.m.duiba.com.cn/sign/component/page?signOperatingId=${signOperatingId}&channel_id=10001&from=login&spm=78211.1.1.1`,
