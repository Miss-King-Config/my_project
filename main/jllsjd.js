...
[task_local]
#金陵连锁酒店
00 00 09,16 * * * , tag=金陵连锁酒店, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 09,16 * * *" script-path=,tag=金陵连锁酒店

===============Surge=================
金陵连锁酒店 = type=cron,cronexp="00 00 09,16 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
金陵连锁酒店 = type=cron,script-path=, cronexpr="00 00 09,16 * * *", timeout=33600, enable=true
*/
...
var appUrlArr = [],
    ...
    user_agent = "Mozilla/5.0 (Linux; Android 9; ELE-AL00 Build/PQ3A.190801.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/116.0.0.0 Mobile Safari/537.36 XWEB/1160065 MMWEBSDK/20231202 MMWEBID/6490 MicroMessenger/8.0.47.2560(0x28002F3B) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx476ba9475c801433";
...
async function get_user_agent() {
    user_agent = "Mozilla/5.0 (Linux; Android " + await random(7, 14) + "; ELE-AL00 Build/PQ3A." + await random(100000, 999999) + "." + await random(100, 999) + "; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/116.0.0.0 Mobile Safari/537.36 XWEB/" + await random(1000000, 9999999) + " MMWEBSDK/20231202 MMWEBID/" + await random(1000, 9999) + " MicroMessenger/8.0." + await random(20, 47) + "." + await random(1000, 9999) + "(0x28002F3B) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx476ba9475c801433";
}
        ...
        $.get(url, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    let data2 = JSON.parse(data);
                    if (data2.success) {
                        let exchangeStatus = data2.data.exchangeStatus;
                        let prizeInfo = data2.data.prizeInfo;
                        if (exchangeStatus != 1 && prizeInfo != null) {
                            let prizeName = prizeInfo.prizeName;
                            $.log(`天降好礼结果 获得${prizeName}`);
                        } else {
                            $.log(`天降好礼结果 未中奖`);
                        }
                    } else {
...
async function user_information() {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://80809.activity-42.m.duiba.com.cn/chw/visual-editor/user/information?_=${timestamp}&`,
            headers: {
                'x-forwarded-for': `${x_forwarded_for}`,
                "user-agent": `${user_agent}`,
                "Referer": `https://80809.activity-42.m.duiba.com.cn/chw/visual-editor/skins?id=150074&from=login&spm=80809.1.1.1`,
                "Cookie": `${Cookie}`
            }
        };
        $.get(url, async (err, resp, data) => {
