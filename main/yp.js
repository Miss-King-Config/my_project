...
//每日任务
async function user_my_level() {
    return new Promise((resolve) => {
        let url = {
            url: `https://ypapp.cnnb.com.cn/yongpai-user/api/user/my_level?userId=${userId}`,
            headers: {
                "system": "Android",
                "version": "28",
                "model": "ELE-AL00",
            ...
            try {
                if (err) {
                    ...
                    if (html.code == 0) {
                        let score = html.data.score;
                        $.log(`现有${score}积分`)
                        let scoreRule = html.data.scoreRule;
                        for (let i = 0; i < scoreRule.length; i++) {
                            let type = scoreRule[i].type;
                            let dayscore = scoreRule[i].dayscore;
                            let usedScore = scoreRule[i].usedScore;
                            if (type == "阅读新闻" && dayscore > usedScore) {
                                await news_detail_news_list(0);
                            } else if (type == "分享新闻" && dayscore > usedScore) {
                                await forward_news_news_list(0);
                            } else if (type == "点赞" && dayscore > usedScore) {
                                await praise_save_news_news_list(0);
                            }
                            if (dayscore == usedScore) {
                                $.log(`每日任务 ${type} 已完成`);
                            } else {
                                $.log(`每日任务 ${type} 未完成`);
                            }
...
//阅读新闻列表
async function news_detail_news_list(channelId) {
...
//分享新闻列表
async function forward_news_news_list(channelId) {
    let currentPage = await random(0, 20);
    return new Promise((resolve) => {
        let url = {
            url: `https://ypapp.cnnb.com.cn/yongpai-news/api/news/list?channelId=${channelId}&currentPage=${currentPage}&timestamp=0`,
...
//点赞新闻列表
async function praise_save_news_news_list(channelId) {
    let currentPage = await random(0, 20);
    return new Promise((resolve) => {
        let url = {
            url: `https://ypapp.cnnb.com.cn/yongpai-news/api/news/list?channelId=${channelId}&currentPage=${currentPage}&timestamp=0`,
            ...
            try {
                if (err) {
                    ...
                    if (html.code == 0) {
                        ...
                        if (body.indexOf("/hdtool/index?id") != -1) {
