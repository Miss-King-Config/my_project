...
[task_local]
#国美小虎币
00 00 2,14 * * * , tag=国美小虎币, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 2,14 * * *" script-path=,tag=国美小虎币

===============Surge=================
国美小虎币 = type=cron,cronexp="00 00 2,14 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
国美小虎币 = type=cron,script-path=, cronexpr="00 00 2,14 * * *", timeout=33600, enable=true
*/

const $ = new Env('国美小虎币');
$.log(`需要新建环境变量: Gm_Cookie\n填写抓包Cookie\n多用户可以用"#" "@" "\\n" 隔开`);
...
async function checkEnv() {
    const Gm_Cookie = ($.isNode() ? (process.env.Gm_Cookie) : ($.getval('Gm_Cookie'))) || "";
    if (!Gm_Cookie) {
        let str = Gm_Cookie ? "" : "Gm_Cookie";
        $.log(`未找到环境变量: ${str}\n`);
        return false;
    }
    if (Gm_Cookie.indexOf('#') != -1) {
        appUrlArrs = Gm_Cookie.split('#');
        $.log(`您选择的是用"#"隔开 Gm_Cookie\n`);
    } else if (Gm_Cookie.indexOf('\n') != -1) {
        appUrlArrs = Gm_Cookie.split('\n');
        $.log(`您选择的是用"\\n"隔开 Gm_Cookie\n`);
    } else if (Gm_Cookie.indexOf('@') != -1) {
        appUrlArrs = Gm_Cookie.split('@');
        $.log(`您选择的是用"@"隔开 Gm_Cookie\n`);
    } else {
        appUrlArrs = [Gm_Cookie];
    }
...
async function sign_in() {
    return new Promise((resolve) => {
        let url = {
            url: `https://club.m.gome.com.cn/mclub/api/sign/in?site=APP`,
            headers: {
