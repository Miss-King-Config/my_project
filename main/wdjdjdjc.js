...
async function checkEnv() {
    const Wdjdjdjc_Cookie = ($.isNode() ? (process.env.Wdjdjdjc_Cookie) : ($.getval('Wdjdjdjc_Cookie'))) || "";
    if (!Wdjdjdjc_Cookie) {
        let str = Wdjdjdjc_Cookie ? "" : "Wdjdjdjc_Cookie";
        $.log(`未找到环境变量: ${str}\n`);
        return false;
    }
    if (Wdjdjdjc_Cookie.indexOf('#') != -1) {
        appUrlArrs = Wdjdjdjc_Cookie.split('#');
        $.log(`您选择的是用"#"隔开 Wdjdjdjc_Cookie\n`);
    } else if (Wdjdjdjc_Cookie.indexOf('\n') != -1) {
        appUrlArrs = Wdjdjdjc_Cookie.split('\n');
        $.log(`您选择的是用"\\n"隔开 Wdjdjdjc_Cookie\n`);
    } else if (Wdjdjdjc_Cookie.indexOf('@') != -1) {
        appUrlArrs = Wdjdjdjc_Cookie.split('@');
        $.log(`您选择的是用"@"隔开 Wdjdjdjc_Cookie\n`);
    } else {
        appUrlArrs = [Wdjdjdjc_Cookie];
    }
