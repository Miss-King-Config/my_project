...
async function getEnvParam(userNum) {
    ...
    if (appUrlArrVal.indexOf("----") != -1) {
        let appUrlArrVal2 = appUrlArrVal.split("----");
        param_str = decodeURIComponent(appUrlArrVal2[0]);
        ...
        user_name = appUrlArrVal2[1];
        phone = appUrlArrVal2[2];
    }
}
                    ...
                    if (html.code == 200) {
                        ...
                        for (let i = 0; i < data2.length; i++) {
                            ...
                            if (prize_id == 44 && prize_name == "电费红包20元" && prize_number > 0) {
                                await $.wait(5000); //等待5秒
                                await PrizeRecord_addPointRecord(prize_id, prize_type);
                            }
                        }
                    } else {
                        $.log("奖品列表 " + html.msg);
...
//兑换
async function PrizeRecord_addPointRecord(prize_id, prize_type) {
    return new Promise((resolve) => {
        let url = {
            url: `https://town.creakeweb.com/api/PrizeRecord/addPointRecord`,
            body: `user_name=${user_name}&phone=${phone}&prize_id=${prize_id}&awardtype=${prize_type}`,
            headers: {
                "Content-Type": 'application/x-www-form-urlencoded; charset=UTF-8',
                "ApiToken": `${ApiToken}`
            }
