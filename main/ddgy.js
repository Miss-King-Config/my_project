...
async function initAccountInfo() {
    for (numUser = 0; numUser < totalUser; numUser++) {
        ...
        if (kongzhi) {
            ...
            await mission_get();
            await $.wait(2000); //等待2秒
            await plant_fertilizer();
            await $.wait(2000); //等待2秒
            await plant_watering();
            await $.wait(2000); //等待2秒
...
//施肥
async function plant_fertilizer() {
    return new Promise((resolve) => {
        let url = {
            url: `https://game.xiaojukeji.com/api/game/plant/fertilizer?wsgsig=${wsgsig}`,
            headers: {
                'Content-Type': 'application/json;charset=UTF-8',
                'D-Header-T': `${token}`
            },
...
//浇水
async function plant_watering() {
    return new Promise((resolve) => {
        let url = {
            url: `https://game.xiaojukeji.com/api/game/plant/watering?wsgsig=${wsgsig}`,
            headers: {
                'Content-Type': 'application/json;charset=UTF-8',
                'D-Header-T': `${token}`
            },
