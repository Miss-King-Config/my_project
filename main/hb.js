...
//修改环境变量的方法，参数1：环境变量名；参数2：需要修改的原值; 参数3：需要修改的新值
function setEnvVar(envName, olddata, newdata) {
    const configPath = '/ql/config/config.sh';
    const configContent = fs.readFileSync(configPath, 'utf-8');
    const pattern = new RegExp(`^export ${envName}='(.*)';`, 'm');
    const match = configContent.match(pattern);
    if (match) {
        const oldValue = match[1]; //另一种读取环境变量的方式
        const newValue = oldValue.replace(olddata, newdata)
                ...
                if (err) {
                    ...
                    if (data2.msgCd == "MCA00000") {
                        ...
                        if (refresh_token_params_json.refreshToken != refreshToken) {
                            ...
                            setEnvVar('Hb_list', refresh_token_params, refresh_token_data);
                        }
