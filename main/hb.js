...
const $ = new Env('和包');
$.log(`需要新建环境变量: Hb_list\n填写抓包 post请求https://mca.cmpay.com/security/login/simplify/refresh-token 格式x-user-id----x-user-no----x-app-deviceid----请求params\n多用户可以用"#" "@" "\\n" 隔开`);
...
async function getEnvParam(userNum) {
    ...
    if (appUrlArrVal.indexOf("----") != -1) {
        x_user_id = appUrlArrVal.split("----")[0];
        x_user_no = appUrlArrVal.split("----")[1];
        x_app_deviceid = appUrlArrVal.split("----")[2];
        refresh_token_params = appUrlArrVal.split("----")[3];
    }
}
