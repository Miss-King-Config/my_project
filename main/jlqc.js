...
async function checkEnv() {
    const Jlqc_token = ($.isNode() ? (process.env.Jlqc_token) : ($.getval('Jlqc_token'))) || "";
    if (!Jlqc_token) {
        let str = Jlqc_token ? "" : "Jlqc_token";
        $.log(`未找到环境变量: ${str}\n`);
        return false;
    }
    if (Jlqc_token.indexOf('#') != -1) {
        appUrlArrs = Jlqc_token.split('#');
        $.log(`您选择的是用"#"隔开 Jlqc_token\n`);
    } else if (Jlqc_token.indexOf('\n') != -1) {
        appUrlArrs = Jlqc_token.split('\n');
        $.log(`您选择的是用"\\n"隔开 Jlqc_token\n`);
    } else if (Jlqc_token.indexOf('@') != -1) {
        appUrlArrs = Jlqc_token.split('@');
        $.log(`您选择的是用"@"隔开 Jlqc_token\n`);
    } else {
        appUrlArrs = [Jlqc_token];
    }
...
const integralCId = "BLqo2nmmoPgGuJtFDWlUjRI2b1b";
const integralSalt = "0]3K@'9MK+6Jf";
...
function get_sign(t) {
    ...
    for (var d in a) {
        if (a[d] == "signDate") {
            c.push(a[d] + "=" + Date.parse(t[a[d]].replace(/-/g, "/")));
        } else {
            c.push(a[d] + "=" + t[a[d]]);
        }
    }
    return toMd5Hex(c.join("&") + integralSalt);
}

//获取时间戳
async function close_currentTime() {
            ...
            try {
                if (err) {
                    ...
                    if (html.code == "success") {
                        var ts = html.data;
                        await userSign_sign(ts);
                    } else {
                        $.log("获取时间戳 " + html.message);
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
...
//签到
async function userSign_sign(ts) {
    var signDate = filterTime()
    var sign_data = {
        "signDate": signDate,
        "ts": ts,
        "cId": `${integralCId}`
    ...
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {
                ...
                "x-data-sign": get_sign(sign_data),
                "token": `${token}`,
                "version": "3.20.0"
            }
        };
