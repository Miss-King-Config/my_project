...
[task_local]
#民生银行苏州分行
00 00 7,11 * * * , tag=民生银行苏州分行, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 7,11 * * *" script-path=,tag=民生银行苏州分行

===============Surge=================
民生银行苏州分行 = type=cron,cronexp="00 00 7,11 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
民生银行苏州分行 = type=cron,script-path=, cronexpr="00 00 7,11 * * *", timeout=33600, enable=true
*/

const $ = new Env('民生银行苏州分行');

$.log(`需要新建环境变量: Msyhszfh_mcpCode\n填写抓包mcpCode\n多用户可以用"#" "@" "\\n" 隔开`);
...
async function checkEnv() {
    const Msyhszfh_mcpCode = ($.isNode() ? (process.env.Msyhszfh_mcpCode) : ($.getval('Msyhszfh_mcpCode'))) || "";
    if (!Msyhszfh_mcpCode) {
        let str = Msyhszfh_mcpCode ? "" : "Msyhszfh_mcpCode";
        $.log(`未找到环境变量: ${str}\n`);
        return false;
    }
    if (Msyhszfh_mcpCode.indexOf('#') != -1) {
        appUrlArrs = Msyhszfh_mcpCode.split('#');
        $.log(`您选择的是用"#"隔开: Msyhszfh_mcpCode\n`);
    } else if (Msyhszfh_mcpCode.indexOf('\n') != -1) {
        appUrlArrs = Msyhszfh_mcpCode.split('\n');
        $.log(`您选择的是用"\\n"隔开: Msyhszfh_mcpCode\n`);
    } else if (Msyhszfh_mcpCode.indexOf('@') != -1) {
        appUrlArrs = Msyhszfh_mcpCode.split('@');
        $.log(`您选择的是用"@"隔开: Msyhszfh_mcpCode\n`);
    } else {
        appUrlArrs = [Msyhszfh_mcpCode];
    }
...
//签到
async function personalcenter_signIn() {
    return new Promise((resolve) => {
        let url = {
            url: `https://suzhsh.cmbc.com.cn/cmb_02_wechat/personalcenter/signIn`,
            body: `wxOpenId`,
            headers: {
                'Upgrade-Insecure-Requests': '1',
                'User-Agent': 'Mozilla/5.0 (Linux; Android 9; ELE-AL00 Build/HUAWEIELE-AL00; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/3262 MMWEBSDK/20211001 Mobile Safari/537.36 MMWEBID/9916 MicroMessenger/8.0.16.2040(0x280010D2) Process/toolsmp WeChat/arm32 Weixin NetType/WIFI Language/zh_CN ABI/arm64',
            ...
            try {
                if (err) {
                    ...
                    if (html.errorCode == 0) {
                        $.log(`签到 ` + html.info);
                        if (html.raffle) {
                            await personalcenter_result();
                            await $.wait(5000); //等待5秒
                        }
                    } else {
                        $.log(`签到 ` + html.errorMsg);
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
...
//抽奖
async function personalcenter_result() {
    return new Promise((resolve) => {
        let url = {
            url: `https://suzhsh.cmbc.com.cn/cmb_02_wechat/personalcenter/result`,
            body: ``,
            headers: {
                'Upgrade-Insecure-Requests': '1',
                'User-Agent': 'Mozilla/5.0 (Linux; Android 9; ELE-AL00 Build/HUAWEIELE-AL00; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/3262 MMWEBSDK/20211001 Mobile Safari/537.36 MMWEBID/9916 MicroMessenger/8.0.16.2040(0x280010D2) Process/toolsmp WeChat/arm32 Weixin NetType/WIFI Language/zh_CN ABI/arm64',
        ...
        $.post(url, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    let html = JSON.parse(data);
                    if (html.errorCode == 0) {
                        $.log(`抽奖 ` + JSON.stringify(html));
                    } else {
                        $.log(`抽奖 ` + html.errorMsg);
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
