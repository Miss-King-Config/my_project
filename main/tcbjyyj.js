...
[task_local]
#汤臣倍健营养家
00 00 5,6 * * * , tag=汤臣倍健营养家, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 5,6 * * *" script-path=,tag=汤臣倍健营养家

===============Surge=================
汤臣倍健营养家 = type=cron,cronexp="00 00 5,6 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
汤臣倍健营养家 = type=cron,script-path=, cronexpr="00 00 5,6 * * *", timeout=33600, enable=true
*/

const $ = new Env('汤臣倍健营养家');
$.log(`需要新建环境变量: Tcbjyyj_openid\n填写抓包openid\n多用户可以用"#" "@" "\\n" 隔开`);
...
//查询积分
async function getTaskPoint() {
    return new Promise((resolve) => {
        let url = {
            url: `https://hdyx.by-health.com/taskCenter/api/task/getTaskPoint?openid=${openid}`,
            headers: {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (html.myTaskPoints) {
                        let myTaskPoints = html.myTaskPoints;
                        $.log(`查询积分 剩余${myTaskPoints}积分`);
                    } else {
...
async function saveSign() {
    return new Promise((resolve) => {
        let url = {
            url: `https://hdyx.by-health.com/taskCenter/api/sign/saveSign`,
            body: `openid=${openid}`,
            headers: {
            ...
            try {
                if (err) {
                    ...
                    if (html.signBaseInfo) {
                        let lastSignPoint = html.signBaseInfo.lastSignPoint;
                        let continuousSignDays = html.signBaseInfo.continuousSignDays;
                        $.log(`签到成功 本周期签到${lastSignPoint}天 连续签到${continuousSignDays}天`);
                    } else {
                        $.log(`签到` + html.message);
                    }
                }
