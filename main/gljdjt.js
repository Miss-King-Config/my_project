...
[task_local]
#格林酒店集团
00 00 7,9 * * * , tag=格林酒店集团, img-url=, enabled=true

================Loon==============
[Script]
cron "00 00 7,9 * * *" script-path=,tag=格林酒店集团

===============Surge=================
格林酒店集团 = type=cron,cronexp="00 00 7,9 * * *",wake-system=1,timeout=33600,script-path=

============小火箭=========
格林酒店集团 = type=cron,script-path=, cronexpr="00 00 7,9 * * *", timeout=33600, enable=true
*/
            ...
            try {
                if (err) {
                    ...
                    if (html.Result) {
                        ...
                        let SilkBag_ActivityID = html.Data.SilkBag.ActivityID;
                        if (IsSign) {
                            $.log(`签到成功 获得${SignPrize}K币`);
                        } else {
                            $.log(`今日已签到 现有${KMoney}K币`);
                        }
                        if (SilkBag_ActivityID == 0) {
                            await $.wait(2000); //等待2秒
